...
function visit(node) {
    if (ts.isCallExpression(node)) {
        ...
        } else if (ts.isPropertyAccessExpression(node.expression) && node.expression.name.getText() === 'from') { // Array.from
...
visit(sourceFile);
console.log(missingKeys.filter(x => !['td', 'th', 'path', 'circle', 'line', 'svg', 'br'].includes(x.type)));
