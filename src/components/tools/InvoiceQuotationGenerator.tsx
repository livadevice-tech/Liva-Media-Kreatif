...
import { InvoiceHistoryView } from './InvoiceManagementModal';
...
export const InvoiceQuotationGenerator: React.FC<InvoiceQuotationGeneratorProps> = ({
  ...
  // Invoice History State & Main Tabs
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-slate-50/50 overflow-hidden animate-fadeIn">
      ...
      <div className="bg-white border-b border-slate-200 px-4 sm:px-6 py-2.5 flex items-center justify-between gap-4 shrink-0 shadow-2xs">
        {/* Left: Tab Switcher (Editor vs Riwayat) */}
        <div className="flex items-center gap-3">
          <div className="inline-flex p-1 bg-slate-100 rounded-xl border border-slate-200/80">
            ...
            >
              ...
              {invoiceHistory.length > 0 && (
                <span className="px-1.5 py-0.2 bg-indigo-50 text-indigo-600 border border-indigo-200 rounded-full text-[10px] font-extrabold">
          ...
          {/* Type Switcher (Invoice vs Quotation) */}
          {activeTab === 'editor' && (
          ...
          {activeTab === 'editor' && (
            <>
              ...
              <button
                ...
                className="px-3 py-1.5 bg-white hover:bg-emerald-50 text-slate-700 hover:text-emerald-700 border border-slate-200 text-xs font-bold rounded-xl shadow-2xs transition-all flex items-center gap-1.5 cursor-pointer"
                title="Simpan invoice ke riwayat"
              >
                {saveSuccess ? (
                ...
                <span>{saveSuccess ? 'Tersimpan' : 'Simpan'}</span>
              </button>
              ...
              <button
                ...
                className="px-3 py-1.5 bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold rounded-xl shadow-2xs transition-all flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
                title="Bagikan link online ke klien"
              >
                {isGeneratingLink ? (
              ...
              <button
                type="button"
                onClick={() => handlePrint()}
                className="px-4 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-xl shadow-xs transition-all flex items-center gap-1.5 cursor-pointer"
              >
        ...
        {activeTab === 'history' ? (
          <div className="max-w-6xl mx-auto">
            <InvoiceHistoryView
              ...
              onDeleteInvoice={handleDeleteInvoiceFromHistory}
