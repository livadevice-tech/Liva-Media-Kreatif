...
export const EmploymentLetterGenerator: React.FC<EmploymentLetterGeneratorProps> = ({
  ...
  // Generate & Save snapshot template to server for external public link
  const handleGeneratePublicLink = async () => {
    try {
      setIsGeneratingLink(true);
      // Generate a random token
      const token = `sk-${Math.random().toString(36).substring(2, 8)}${Date.now().toString(36)}`;
      const templateData = {
      ...
      await appApi.saveSettings(`public_sk_template_${token}`, templateData);
      const generatedLink = `${window.location.origin}/?sk_token=${token}`;
      ...
      <div className="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between gap-4 shrink-0 shadow-xs">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            title="Bagikan link ke pihak eksternal/karyawan untuk mengisi Nama, NIK, dan Jabatan sendiri"
          >
            {isGeneratingLink ? (
            ...
            <span>Bagikan Link Eksternal</span>
          </button>
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6 border border-slate-200 animate-scaleUp">
            <div className="flex items-center justify-between pb-4 border-b border-slate-100 mb-4">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="text-sm font-bold text-slate-900">Bagikan Link ke Eksternal</h3>
                  <p className="text-[11px] text-slate-500">Tautan pengisian mandiri surat keterangan kerja</p>
                </div>
              </div>
              <button
                type="button"
            ...
            <div className="space-y-4 text-xs text-slate-600">
              <p className="leading-relaxed">
                Karyawan / pihak eksternal dapat membuka link ini tanpa perlu login. Mereka hanya perlu mengisi 
                <span className="font-semibold text-slate-900"> Nama Karyawan, NIK, dan Jabatan</span>. 
                Kop surat resmi, stempel, dan tanda tangan digital perusahaan yang sudah Anda setting akan terpasang otomatis.
              </p>
              ...
              {/* Features list */}
              <div className="bg-emerald-50/70 border border-emerald-200/80 rounded-xl p-3 space-y-1.5 text-[11px] text-emerald-900">
                <div className="font-bold flex items-center gap-1.5 text-emerald-800">
                  <Sparkles className="w-3.5 h-3.5" />
                  <span>Keuntungan Link Eksternal:</span>
                </div>
                <div className="flex items-start gap-1.5">
                  <span>•</span>
                  <span>Dapat dibuka langsung di browser HP atau Komputer tanpa akun.</span>
                </div>
                <div className="flex items-start gap-1.5">
                  <span>•</span>
                  <span>Data Kop PT, Logo, dan Tanda Tangan Anda terkunci aman sesuai settingan saat ini.</span>
                </div>
                <div className="flex items-start gap-1.5">
                  <span>•</span>
                  <span>Karyawan bisa langsung cetak atau simpan file PDF resmi secara mandiri.</span>
                </div>
              </div>
            </div>
