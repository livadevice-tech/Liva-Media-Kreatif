...
const targetStr = `if (!window.confirm("Auto Generate akan membuat jadwal harian untuk seluruh sesi di bulan ini (dan menghapus jadwal lama yang berpotongan). Lanjutkan?")) return;
...
const replacementStr = `requestConfirm("Konfirmasi Auto Generate", "Auto Generate akan membuat jadwal harian untuk seluruh sesi di bulan ini (dan menghapus jadwal lama yang berpotongan). Lanjutkan?", () => {
                               const year = operatorSelectedYear;
                               const month = operatorSelectedMonth; // 0-indexed
...
if (code.includes('if (!window.confirm("Auto Generate akan membuat jadwal harian')) {
    code = code.replace(targetStr, replacementStr);
    fs.writeFileSync('src/App.tsx', code);
    console.log("Replaced with requestConfirm!");
} else {
